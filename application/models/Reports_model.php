...
class Reports_model extends CI_Model 
{
	...
	$this->db->select('users.name,users.address,users.current_city,users.current_state,users.current_country,users.current_pincode,users.parent_name,users.mobile,users.mobile,student_fee.fee_amount')->from('student_fee');
	$this->db->join('users ', 'users.u_id = student_fee.s_id', 'left');
	$this->db->where('student_fee.status',1);
	$this->db->group_by('student_fee.s_id');
	return $this->db->get()->result_array();
	...
	$this->db->select('users.name,users.address,users.current_city,users.current_state,users.current_country,users.current_pincode,users.parent_name,users.mobile,users.mobile,SUM(student_fee.pay_amount) as due_amount,student_fee.fee_amount')->from('student_fee');
	$this->db->join('users ', 'users.u_id = student_fee.s_id', 'left');
	$this->db->where('student_fee.status',1);
	$this->db->group_by('student_fee.s_id');
	return $this->db->get()->result_array();	
